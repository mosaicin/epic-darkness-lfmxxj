...
const TodoList = () => {
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      <ul className="mt-4">
        {todos.map(todo => (
          <li key={todo.id} className="flex items-center justify-between p-2 border border-gray-300 rounded-lg mb-2">
            <span
            ...
            <div className="flex items-center">
              <button
                ...
                className={`mr-2 ${todo.completed ? 'bg-green-500' : 'bg-gray-300'} hover:bg-green-700 text-white font-bold py-1 px-2 rounded-lg focus:outline-none focus:ring focus:ring-green-500`}
              >
                {todo.completed ? 'Completed' : 'Mark as completed'}
              </button>
              <button
                onClick={() => handleDelete(todo.id)}
                className="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded-lg focus:outline-none focus:ring focus:ring-red-500"
              >
                Delete
              </button>
            </div>
